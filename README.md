# FoldFlight Lab

Independent origami and paper-flight editorial site.

Production: https://foldflightlab.github.io/

## Commercial state
- Amazon.de Associates tracking ID: `foldflightlab-21`
- Commercialization: **enabled**
- Affiliate links are labeled and use `rel="sponsored nofollow noopener"`
- Required Amazon Associate disclosure is published site-wide and on the dedicated disclosure page

## Editorial standard
- Original decision frameworks and educational synthesis
- Primary-source cross-checking where useful (for example NASA, Smithsonian, OrigamiUSA)
- No copied Amazon prices, ratings, customer reviews, product images, or stock claims
- No fabricated product testing
- No own analytics, advertising pixels, or visitor accounts at this stage
- Consent-first Flight Fix Dispatch signup using visitor-initiated email to the published FoldFlight Lab Gmail address
- Subscriber addresses are used only for the FoldFlight Lab updates described at signup; they are not sold, licensed, shared for another party's marketing, or repurposed outside that consent

## Production controls
- GitHub Pages over HTTPS
- GitHub Actions pinned to immutable commit SHAs
- CI checks legacy branding/URLs, affiliate configuration, approved public email, SEO metadata, sitemap integrity, and broken internal links
- Private credentials, tax records, banking data, identity documents and admin controls must never be committed here
- Historic Git PII is audited separately; current legal contact details remain intentionally public only where required by the live Impressum/privacy notice
